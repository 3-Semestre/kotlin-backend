...
interface AgendamentoRepository : JpaRepository<Agendamento, Int> {
    ...
    @Query("SELECT a FROM Agendamento a JOIN FETCH a.historico h WHERE a.professor.id = :userId")
    fun findAgendamentosByFkProfessor(@Param("userId") userId: Int): List<Agendamento>
